...
final class AutostartService {
    ...
    AutostartService(JManhuntPlugin plugin, MessageService messages, PlayerStateStore playerStates,
    ...
    void cancelAutostartCountdown(int lobbyId, boolean announce) {
    ...
    void cancelAllAutostartCountdowns(boolean announce) {
        for (int lobbyId : List.copyOf(autostartCountdowns.keySet())) {
     ...
     * members.
     */
    void broadcastAutostartShortfalls() {
        if (!plugin.getConfig().getBoolean("settings.autostart.enabled", false)) {
            return;
        }
