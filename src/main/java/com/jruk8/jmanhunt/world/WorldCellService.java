...
final class WorldCellService {
    ...
    WorldCellService(JManhuntPlugin plugin, ConfigService configService, EngineStateRepository engineState,
    ...
    void setMatchRunningSupplier(BooleanSupplier matchRunning) {
     ...
     */
    OptionalLong onMatchStart(List<Player> participants, List<Player> spectators,
     ...
     */
    void teleportJoinersToCell(List<Player> joiners, long cellIndex) {
        if (joiners.isEmpty()) {
            return;
        }
        WorldEngineConfig config = WorldEngineConfig.fromConfig(plugin.getConfig());
    ...
    Optional<Location> cellRoot(long cellIndex) {
        WorldEngineConfig config = WorldEngineConfig.fromConfig(plugin.getConfig());
        if (!config.enabled()) {
