...
/**
 * Resolves {@code %jmanhunt_<key>%} values with no PlaceholderAPI
 * dependency, so modifier commands can parse them in-house when PAPI
 * is absent. The PAPI expansion shell delegates here too: one
...
public final class JManhuntPlaceholders {
    ...
    /**
     * Resolves one expansion key. Lobby keys work player-less; player
     * keys yield empty without a player and null when unknown.
     */
